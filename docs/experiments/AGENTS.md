...
| `telemetry-fitness.md` | — (standalone driver `scripts/telemetry_fitness.py`, no conf/experiment entry) | The goodness-of-fit HARNESS for a candidate rotor-speed trajectory (issue #17 phases 6a-6d): FOUR components at fixed degrees of freedom, held-out harmonics/channels/time, all four §B controls, residual decomposition + bootstrap. Design, synthetic acceptance tests, the 6c campaign, and the 6d sensitivity fix — **read § "Phase 6d" first if you are citing a number**: the first three components are shares of one in-band power and saturate, the ridge component is the one that reads a line against a local floor |
