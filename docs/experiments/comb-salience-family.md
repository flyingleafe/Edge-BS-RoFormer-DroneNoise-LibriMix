...
## Final: the gain is architectural, and training does not add to it

Run 3 used the rate-conditioned head (137 parameters, a weight per
(harmonic, candidate rate) from a small MLP) with selection on the decode metric
rather than the loss. It picked step 500. Re-scored with the Viterbi decoder —
the one actually deployed, which the in-loop evaluation does not use:

| regime | untrained | trained | classical | PIT-MSE regression |
|---|---|---|---|---|
| identical | **1.216** | 1.237 | 1.132 | 4.643 |
| tight | **0.907** | 0.912 | 1.066 | 4.284 |
| close | **0.420** | 0.669 | 1.056 | 4.361 |
| typical | **0.043** | 0.052 | 1.254 | 4.374 |
| wide | **0.038** | 0.048 | 0.374 | 4.127 |
| typical-fast | **3.062** | 3.077 | 3.399 | 5.542 |
| typical-idle | **5.200** | 5.983 | 8.885 | 6.428 |

The untrained corner case is better or tied on every cell. Training helped under
the ARGMAX decoder it was selected with (close 0.542 -> 0.459, wide 0.691 ->
0.214) and that advantage did not survive the switch to the Viterbi decoder —
a selection/deployment mismatch, and the third instance of the same class of
error in this campaign.

**But there is a deeper reason, and it is not a defect.** The benchmark's clips
are generated by exactly the model the Whittle score assumes: discrete lines at
integer multiples of a rate, plus a noise floor. On data from that model the
score IS the likelihood statistic, so it is already near-sufficient and there is
nothing for a learned head to recover. A learned head can only overfit the
sampling noise, which is what the runs show.

That predicts where learning WOULD pay: real recordings, where the generative
model is wrong in specific ways the score cannot express — broadband rotor
noise, reflections, wind, per-harmonic amplitude drift, and harmonics that
wander off the exact integer grid. None of this has been tested on real audio,
and that is the single measurement that would decide it.

## Summary against the goal

**What the regression family cannot do, by construction:** it regresses R rates
under PIT-MSE so its optimum is the conditional mean (measured: error flat at
4.1-4.6 rev/s regardless of rotor spread, and no better than predicting the
per-frame centre at the training-matched spread); its convolutions share weights
across translations while a comb at rate `r` is a DILATION of one at `r'`, so
there is no weight sharing across the problem's actual symmetry; and it has no
explain-away loop and no temporal MAP.

**The family that contains the classical method:** `CombGather` reads the
spectrum at every harmonic of every candidate rate, `CombScoreHead` scores those
readings, an unrolled peel supplies model order and a Viterbi path supplies
continuity. The corner case is exact to 4.4e-15, and the learned heads
initialize to it.

**Parity:** exceeded. Six of seven cells beat the classical scan and the seventh
is within 7%; the training-matched cell is 0.043 rev/s against 1.254 classical
(29x) and 4.374 for the regression family (102x).
