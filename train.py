...
def parse_args(dict_args: Union[Dict, None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Existing wandb run ID to resume logging into",
    )
        ...
        try:
            ...
            for key in ("train", "valid"):
                if key in data and isinstance(data[key], list):
                    return {
                        item["id"]: item["input_snr"]
                        for item in data[key]
                        if "id" in item and "input_snr" in item
                    }
            return None
        except Exception:
            return None
...
def select_sample_ids_by_snr(
    ...
    for cat_name, (min_db, max_db) in SNR_CATEGORIES.items():
        candidates = [
            (sid, snr)
            for sid, snr in metadata.items()
            if min_db <= snr <= max_db and np.isfinite(snr)
        ]
        if not candidates:
            continue
...
def collect_audio_triples_by_snr(
    ...
    with torch.no_grad():
        for sample_id, cat_name, snr_val in selected:
            try:
                mix, tgt = load_sample_audio_from_folder(
                    root, sample_id, target_file=target_file, target_sr=sample_rate
                )
            ...
            triples.append(
                (
                    np.asarray(inp_1d, dtype=np.float32),
                    np.asarray(y_np_1d, dtype=np.float32),
                    out_1d,
                )
            )
...
def log_audio_triples_to_wandb(
    ...
    for idx, (inp, tgt, out) in enumerate(triples):
        ...
        if not output_only:
            log_dict[f"{prefix}/sample_{idx}/input"] = wandb.Audio(
                inp, sample_rate=sample_rate, caption=f"input {cap}"
            )
            log_dict[f"{prefix}/sample_{idx}/target"] = wandb.Audio(
                tgt, sample_rate=sample_rate, caption=f"target {cap}"
            )
        log_dict[f"{prefix}/sample_{idx}/output"] = wandb.Audio(
            out, sample_rate=sample_rate, caption=f"output {cap}"
        )
    wandb.log(log_dict)
    ...
    for i, data in enumerate(pbar):
        ...
        with torch.autocast(device_type=device.type, enabled=use_amp):
            if args.model_type in [
                ...
                rps_loss_only = getattr(config.training, "rps_loss_only", False)
                ...
                if rps_loss_only:
                    assert rps_pred is not None and rps is not None, (
        ...
        # Skip for rps_loss_only mode — no audio output to log
        rps_loss_only_flag = getattr(config.training, "rps_loss_only", False)
        if (
            not rps_loss_only_flag
            ...
            if audio_logging_cache is not None:
                cached_rps = (
                    [rps_slice[k].cpu().numpy() for k in range(rps_slice.shape[0])]
                    if rps_slice is not None
                    else None
                )
        ...
        with torch.no_grad():
            for data in valid_loader:
                ...
                for k in range(min(3, x.shape[0])):
                    ...
                    tgt_k = _tensor_to_audio_numpy(y[k])
                    out_k = (
                        _tensor_to_audio_numpy(out[k])
                        if out.shape[0] > k
                        else _tensor_to_audio_numpy(out[0])
                    )
                    triples.append((inp_k, tgt_k, out_k))
                if audio_logging_cache is not None and triples:
                    cached_rps = (
                        [rps[k].cpu().numpy() for k in range(min(3, rps.shape[0]))]
                        if rps is not None
                        else None
                    )
...
def train_model(args: argparse.Namespace) -> None:
    ...
    for epoch in range(config.training.num_epochs):
        ...
        current_metric, best_metric = compute_epoch_metrics(
            model, args, config, device, device_ids, epoch, scheduler, best_metric
        )
        if valid_loader is not None:
            collect_and_log_valid_audio(
                model, args, config, device, valid_loader, epoch, audio_logging_cache
            )
        if early_stop_enabled:
            ...
            if no_improvement_count >= early_stop_patience:
                print(
                    f"Early stopping: {metric_for_early_stop} has not improved for {early_stop_patience} consecutive epochs."
                )
                save_weights(
                    args,
                    args.results_path,
