...
def generate_comparison_plot(models, output_dir, plot_name="comparison.png"):
    ...
    cmd = (
        ["python", "generate_comparison.py", "--models"]
        + models
        + ["--output_dir", str(output_dir), "--plot_name", plot_name]
    )
    run_command(cmd)
...
def main():
    ...
    parser.add_argument("--models", nargs="+", help="Models to compare for results slides")

    parser.add_argument("--samples", nargs="+", help="Sample IDs for audio comparison")
