...
7. **Analyze results.**
   - Sync first (mandatory): `omnirun pull <job>` for omnirun jobs (collects `results/**`); legacy rsync fallback `./scripts/sync_results.sh`
...
- **Always sync results before analysis** — `omnirun pull <job>` (legacy: `./scripts/sync_results.sh`)
