...
CHECKPOINT = "results/rps_exp_simple_conv/best_simple_conv.pt"
DREGON_VALID = ROOT / "datasets/DREGON-LM/valid"
...
def fig_training_curves():
    ...
    if not csv_path.is_file():
        csv_path = ROOT / "results/rps_exp_simple_conv/training_log.csv"
...
def fig_qualitative_examples():
    ...
    all_samples = {s.tags.get("id", ""): s
                   for s in load_input_set(str(DREGON_VALID))}
    ...
    for col, sid in enumerate(sample_ids):
        ...
        ax.imshow(log_mag, origin="lower", aspect="auto",
                  ...
                  cmap="hot", vmin=np.percentile(log_mag, 2), vmax=np.percentile(log_mag, 99))
        ax.set_ylim(0, 4)
        ax.set_ylabel("freq [kHz]" if col == 0 else "")
        ax.set_title(f"sample {sid.split('_')[1]}")
        ax.set_xticklabels([]); ax.grid(False)
        ...
        for r in range(4):
            ax.plot(frame_times[:T], gt[r, :T], ":", color=ROTOR_COLORS[r],
                    lw=0.5, alpha=0.55, label=f"GT R{r+1}" if col == 0 else None)
            ax.plot(frame_times[:T], pred_arr[r, :T], "-", color=ROTOR_COLORS[r],
                    lw=0.5, alpha=0.75, label=f"pred R{r+1}" if col == 0 else None)
        ax.set_xlabel("time [s]")
        if col == 0: ax.set_ylabel("rotor speed [rev/s]")
        ax.set_xlim(0, dur)
...
def fig_highsnr_per_sample():
    data_path = ROOT / "results/rps_high_snr_analysis.json"
    ...
    ax.annotate("drone-landing regime\n(RPS→0)", xy=(outlier_idx, sc_mse[outlier_idx]),
...
def fig_highsnr_outlier():
    ...
    rps_stft = np.array([np.interp(np.arange(n_frames) * HOP / TARGET_SR,
                                    np.arange(rps_chunk.shape[1]) / motor_sr,
                                    rps_chunk[r]) for r in range(4)])
    ...
    spec = np.abs(np.fft.rfft(
        ...
        np.hanning(N_FFT), axis=-1))
    log_mag = np.log1p(spec.T)
    ax.imshow(log_mag, origin="lower", aspect="auto", extent=[0, dur, 0, TARGET_SR / 2 / 1000],
              cmap="hot", vmin=np.percentile(log_mag, 2), vmax=np.percentile(log_mag, 99))
    ax.set_ylim(0, 4); ax.set_ylabel("freq [kHz]")
    ax.set_title("outlier clip ($t \\approx 38.6$ s) — drone landing")
    ax.set_xticklabels([]); ax.grid(False)

    ax = axes[1]
    for r in range(4):
        ax.plot(np.linspace(0, dur, rps_stft.shape[1]), rps_stft[r], ":",
                color=ROTOR_COLORS[r], lw=0.5, alpha=0.55)
        ax.plot(np.linspace(0, dur, pred_arr.shape[1]), pred_arr[r], "-",
                color=ROTOR_COLORS[r], lw=0.5, alpha=0.75)
    ax.set_xlabel("time [s]"); ax.set_ylabel("rotor speed [rev/s]")
    ax.set_xlim(0, dur)
...
def fig_full_sequence():
    ...
    audio_crop = audio_full[:, int((t0 - audio_ts[0]) * file_sr):int((t1 - audio_ts[0]) * file_sr)]
    ...
    if peak > 0: audio_mono = audio_mono / peak * 0.9
    ...
    fig = PLOT_TYPES["rps_prediction.full_sequence"](
        audio=audio_mono.squeeze().numpy(), rps_gt=gt, rps_pred=pred_arr,
    )
...
if __name__ == "__main__":
    ...
    all_figs = {"training_curves", "qualitative", "highsnr_per_sample",
                "highsnr_outlier", "full_sequence"}
    ...
    for name in sorted(to_run):
        ...
        try:
            {"training_curves": fig_training_curves,
             ...
             "full_sequence": fig_full_sequence}[name]()
