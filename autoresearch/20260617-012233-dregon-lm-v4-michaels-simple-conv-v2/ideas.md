...
| H5 | submitted | simple_conv_v2_magphase | Keep the winning `simple_conv_v2` encoder/pool/BiGRU pattern but use log-magnitude plus phase (`cos`, `sin`) input. | Phase may expose frame-to-frame harmonic motion while preserving the BiGRU temporal bias that beat attention variants. | Existing smaller magphase model exists, but this v2-sized variant may overfit or add noisy phase cues. |
| H6 | submitted | simple_conv_v2_dual_pool | Concatenate attention frequency pooling with simple mean frequency pooling before the BiGRU head. | Attention may focus on harmonics while mean pooling preserves broad energy/context lost by attention-only pooling. | Slightly larger head; benefit may be redundant. |
| H7 | proposed | simple_conv_v2_gru96 | Increase BiGRU hidden size from 64 to 96 while keeping the rest of `simple_conv_v2` unchanged. | If baseline is capacity-limited in temporal tracking, modest GRU capacity may improve without changing inductive bias. | May overfit tiny validation set or slow training; hyperparameter-ish, lower priority than H5/H6. |
