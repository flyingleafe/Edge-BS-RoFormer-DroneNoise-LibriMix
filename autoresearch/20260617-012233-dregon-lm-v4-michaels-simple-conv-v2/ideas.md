...
| H11 | submitted/running | simple_conv_v2_uni_gru128 | Replace BiGRU with a unidirectional GRU whose hidden size is 128, matching the baseline BiGRU output width (`2*64`). | H8 may have failed because the one-direction head had half the temporal state/output capacity of the BiGRU. | Smoke-tested; running after >10 s check. Still head-only causal; could overfit or become unstable like H8. |
| H12 | submitted/running | simple_conv_v2_uni_gru128_norm | Same as H11, but add GroupNorm after the causal Conv1d prenet. | H8 produced NaNs; normalizing the prenet may stabilize high-capacity causal recurrence under the fixed LR/AMP setup. | Smoke-tested; running after >10 s check. Normalization may damp useful scale cues; still not fully waveform-causal. |
