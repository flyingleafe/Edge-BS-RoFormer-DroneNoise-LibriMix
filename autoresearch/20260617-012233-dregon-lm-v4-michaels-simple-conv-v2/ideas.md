...
| H7 | submitted/pending | simple_conv_v2_gru96 | Increase BiGRU hidden size from 64 to 96 while keeping the rest of `simple_conv_v2` unchanged. | If baseline is capacity-limited in temporal tracking, modest GRU capacity may improve without changing inductive bias. | Smoke-tested; job pending due to QOS GPU limit. |
