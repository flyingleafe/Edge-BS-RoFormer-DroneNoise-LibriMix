---
experiment: sal150_comb
training_config: conf/experiment/sal150_comb.yaml
...
multif0_salience on the analytic static-comb curriculum, WIDE 0-150 rev/s output grid.
...
row, flight MAE 4.01):
1. The stream, from the honest real base to conf/online_mix/m3abl_comb_s1_dload.yaml.
...
synthetic family (conf/data/sal_comb_synthval.yaml) — the row asks
...
Hydra wiring — data `sal_comb_synthval` · model `multif0_salience_w150` · loss `salience_bce_w150` · metrics `salience_bce_w150`. Train with `python train.py experiment=sal150_comb`.
