---
experiment: r2hb_tr_nogate
training_config: conf/experiment/r2hb_tr_nogate.yaml
...
`snr_ref_floor_rms: 0.02`) — at the fixed original architecture, the IF
front-end transformer (`simple_conv_v2_transformer_if`). The HB grid already
...
and `r2hb_gru_nogate` complete the trio. Validation stays the fixed
full-envelope real split `dload:DREGON-LM-V4-michaels-valid-full`. Train:
`python train.py experiment=r2hb_tr_nogate`.
