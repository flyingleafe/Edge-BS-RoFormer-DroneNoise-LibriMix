---
experiment: m3cur_transformer_s2
training_config: conf/experiment/m3cur_transformer_s2.yaml
batch: docs/experiments/generator-refined-labels.md
---

# `m3cur_transformer_s2`

## Motivation

Stage 2 of the M3 curriculum for the IF front-end transformer (`simple_conv_v2_transformer_if`): fine-tune on REAL data,
warm-started from `m3cur_transformer_s1`'s `best.ckpt`. Measures what the synthetic
...
the RPS prior stage 1 paid to break. Model `simple_conv_v2_transformer_if`, loss `pit_mse`, metrics
`rps`, warm start
`checkpoint: r2://ml-data/artifacts/m3cur_transformer_s1/checkpoints/best.ckpt`
...
Train: `python train.py experiment=m3cur_transformer_s2` (after stage 1 uploads
best.ckpt).
