---
experiment: r3hb_gru
training_config: conf/experiment/r3hb_gru.yaml
...
`m3cur_unigru128_s1`'s `best.ckpt`, the same checkpoint the as-run
`m3cur_unigru128_s2` used, so the synthetic pre-training is reused, not repeated.
...
`python train.py experiment=r3hb_gru`.
