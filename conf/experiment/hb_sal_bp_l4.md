---
experiment: hb_sal_bp_l4
training_config: conf/experiment/hb_sal_bp_l4.yaml
...
Level L2 of the Basic Pitch row in Block S of the paper regime matrix (see the
...
ORIGINAL INPUT. Its L0 is the old `hb_sal_bp`; the row has no L1 outside June.
The regime is R4 as `hb_sal_bp` ran it, the `hb_silence_dload.yaml` pool.

`hb_sal_bp` reads ONE SHARED salience map and decodes it by threshold plus
...
Architecture: `basic_pitch_salience_l4` — the Bittner contour branch at native
16 kHz, 264 contour bins, 36 bins per octave, `fmin` 27.5 Hz. The INPUT is
`hb_sal_bp`'s exactly: same CQT and harmonic stacking, same contour trunk, same
[16000, 256] time grid. Only `BasicPitch.contour_out` and the super-resolution
head widen to four channels, which is 1182 more parameters, and the output
becomes four per-rotor layers on a LINEAR 0-150 rev/s grid of 300 bins
(`superres_out: true`; the CRF band and the vertex fit are both defined on a
uniform axis, and the contour grid is log-spaced).
...
bin below the input `fmin` reads the same input bin: the band under 27.5 rev/s
...
share from above. Moving the input would buy the band; L2 may not, because it
isolates the output representation. L3, the comb gather on the linear STFT, is
the level that fixes it, and Basic Pitch has no L3.
...
Data: unchanged from `hb_sal_bp` — `conf/online_mix/hb_silence_dload.yaml`,
validated on the frozen full-envelope real split
`dload:DREGON-LM-V4-michaels-valid-full`.

Train with `python train.py experiment=hb_sal_bp_l4`.
