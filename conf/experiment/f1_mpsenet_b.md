---
experiment: f1_mpsenet_b
training_config: conf/experiment/f1_mpsenet_b.yaml
...
MP-SENet (parallel magnitude+phase, generator-only ~1.7M), Pass B (all harmonic noises (category-uniform)) of the F1 blind speech-enhancement baseline
...
model `f1_mpsenet` · loss `masked_mse` · metrics `separation_basic`
(eval with `metrics=separation_full` for PESQ/eSTOI). Train with
`python train.py experiment=f1_mpsenet_b`, evaluate with
`python eval.py experiment=f1_mpsenet_b metrics=separation_full`.
