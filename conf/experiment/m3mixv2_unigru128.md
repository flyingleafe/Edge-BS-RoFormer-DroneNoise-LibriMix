---
experiment: m3mixv2_unigru128
training_config: conf/experiment/m3mixv2_unigru128.yaml
...
Re-runs one-stage mixed training for the unidirectional GRU-128 head with a validation set that matches
the training mixture, because the original verdict measured the stopping rule
rather than the method.

`m3abl_mixed_unigru128` scored 179.7 / 8.67 at 25 epochs and stopped there. The same
...
Hydra wiring — data `m3mix_matchedval` · model `simple_conv_v2_uni_gru128` · loss `pit_mse` ·
metrics `rps`. Train with `python train.py experiment=m3mixv2_unigru128`.
