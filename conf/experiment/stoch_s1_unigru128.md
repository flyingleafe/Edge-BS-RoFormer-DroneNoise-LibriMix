---
experiment: stoch_s1_unigru128
training_config: conf/experiment/stoch_s1_unigru128.yaml
...
split, on the causal unidirectional GRU trunk. The whole noise pool is the
...
The comparison row is `m3abl_comb_unigru128_s1` — the analytic static comb as the
whole pool on the same trunk, the same optimizer, the same validation split and
the same three augmentation blocks. It reaches 212 validation PIT-MSE, against
...
Data `stoch_s1`, model `simple_conv_v2_uni_gru128`, loss `pit_mse`, metrics `rps`, batch 128
...
Train: `python train.py experiment=stoch_s1_unigru128`.
