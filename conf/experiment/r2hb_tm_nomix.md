---
experiment: r2hb_tm_nomix
training_config: conf/experiment/r2hb_tm_nomix.yaml
...
the magnitude transformer (`simple_conv_v2_transformer`). This file is `tm_r2hb_nogate` with the
...
**Read the A/B against `real_r4_tm`, not against the source row.** The
...
no warm-up. The exact with-speech twin would be `tm` on
...
Train: `python train.py experiment=r2hb_tm_nomix`.
