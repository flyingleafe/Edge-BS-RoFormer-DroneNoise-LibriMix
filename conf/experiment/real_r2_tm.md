---
experiment: real_r2_tm
training_config: conf/experiment/real_r2_tm.yaml
...
Rung 2 of the reality ladder — ONE rig (DREGON), ALL 8 microphones — on
the magnitude-front-end transformer (`simple_conv_v2_transformer`).
...
This file is `tm_r2hb_nogate` with the training stream replaced by
`conf/online_mix/real_r2_dload.yaml` and the monitor changed to `mae_frame`, the
...
Train: `python train.py experiment=real_r2_tm`.
