---
experiment: f1_sgmse_b
training_config: conf/experiment/f1_sgmse_b.yaml
...
NCSN++ backbone ~65M, trained from scratch at native 16 kHz), Pass B (all harmonic noises (category-uniform)) — the diversity arm of the F1
...
`python scripts/train_sgmse.py --experiment f1_sgmse_b --wandb` (train.py's discriminative loop cannot host score-based diffusion), evaluate with
`python eval.py experiment=f1_sgmse_b metrics=separation_full`.
