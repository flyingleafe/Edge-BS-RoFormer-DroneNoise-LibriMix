---
experiment: a1_edge_bs_rof_fa
training_config: conf/experiment/a1_edge_bs_rof_fa.yaml
...
+ Flash Attention, on DN-LM (Paper 1). See REPLICATION.md § A1.

Hydra wiring — data `dn_lm` · model `a1_edge_bs_rof_fa` · loss `masked_mse` · metrics `separation_basic`. Train with `python train.py experiment=a1_edge_bs_rof_fa`,
evaluate with `python eval.py experiment=a1_edge_bs_rof_fa`.
