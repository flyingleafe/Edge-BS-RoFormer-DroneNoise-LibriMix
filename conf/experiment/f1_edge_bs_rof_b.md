---
experiment: f1_edge_bs_rof_b
training_config: conf/experiment/f1_edge_bs_rof_b.yaml
...
Edge-BS-RoFormer (band-split transformer, Paper 1 in-house SOTA), Pass B (all harmonic noises (category-uniform)) of the F1 blind speech-enhancement baseline
...
model `a1_edge_bs_rof_fa` · loss `masked_mse` · metrics `separation_basic`
(eval with `metrics=separation_full` for PESQ/eSTOI). Train with
`python train.py experiment=f1_edge_bs_rof_b`, evaluate with
`python eval.py experiment=f1_edge_bs_rof_b metrics=separation_full`.
