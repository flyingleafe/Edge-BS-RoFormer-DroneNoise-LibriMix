---
experiment: salv2_scv2_stoch_mix
training_config: conf/experiment/salv2_scv2_stoch_mix.yaml
batch: docs/experiments/salv2-speech-and-objective-grid.md
---

# `salv2_scv2_stoch_mix`
...
for the two salience ports. This cell is **SimpleConvV2, the regressor baseline (BiGRU head, direct rev/s output)** on the **stochastic rotor-noise family**,
**with LibriSpeech mixed in at -30 to 0 dB SNR**, trained on permutation-invariant MSE on the rotor speeds.
...
Hydra wiring — data `salv2_stoch_mix` · model `simple_conv_v2` · loss `pit_mse` · metrics
`rps`. Train with `python train.py experiment=salv2_scv2_stoch_mix`.
...
The monitor is `mae_frame` — PIT per-frame absolute error in rev/s — and NOT the
