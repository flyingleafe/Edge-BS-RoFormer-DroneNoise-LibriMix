---
experiment: e9_hard_transformer_ft_real
training_config: conf/experiment/e9_hard_transformer_ft_real.yaml
batch: docs/experiments/e9-hard-combined.md
---

# `e9_hard_transformer_ft_real`

## Motivation

Stage 2 of the E9 sim->real curriculum for `simple_conv_v2_transformer`: fine-tune on **real** data
(DREGON in_flight_noise + Michael's FLY125 online mix, no augmentation),
warm-started from the stage-1 `e9_hard_transformer` `best.ckpt` (converged on the
...
FLY125 ground warm-up is excluded), model `simple_conv_v2_transformer`, loss `pit_mse`, metrics
`rps`. Warm-start `checkpoint: r2://ml-data/artifacts/e9_hard_transformer/checkpoints/best.ckpt`
...
Train: `python train.py experiment=e9_hard_transformer_ft_real` (after stage 1
uploads best.ckpt).
