---
experiment: jhtr_s1_nomix
training_config: conf/experiment/jhtr_s1_nomix.yaml
parent: salv2_scv2_comb_nomix
---
...
Hydra inherits [`salv2_scv2_comb_nomix.yaml`](salv2_scv2_comb_nomix.yaml), then applies only the declared JHTR overrides. Inherit the complete SALV2 recipe: unchanged synthetic policy and seeds, 4 s training / 8 s validation, batch 32, 16,000 samples/validation, 200 epochs maximum, patience 20 and `mae_frame` selection. Mix retains paired speech/no-speech validation; nomix retains its fixed 256-frame set. Start from scratch.
