---
experiment: r4hb_gru
training_config: conf/experiment/r4hb_gru.yaml
...
curriculum — at the fixed original architecture, the causal unidirectional-GRU head
(`simple_conv_v2_uni_gru128`). Stage 1 is unchanged: this run warm-starts from
`m3abl_comb_unigru128_s1`'s `best.ckpt`, the same checkpoint the as-run
`m3abl_comb_unigru128_s2` used, so the synthetic pre-training is reused, not repeated.
...
`python train.py experiment=r4hb_gru`.
