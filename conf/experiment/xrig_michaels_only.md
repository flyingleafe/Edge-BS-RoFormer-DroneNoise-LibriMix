...
training_config: conf/experiment/xrig_michaels_only.yaml
...
This run is `r4hb_scv2` with DREGON removed from the real pool and nothing else
changed: same warm start, same optimizer, same validation. Its DREGON column is
...
Train: `python train.py experiment=xrig_michaels_only`.
