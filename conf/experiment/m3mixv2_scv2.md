---
experiment: m3mixv2_scv2
training_config: conf/experiment/m3mixv2_scv2.yaml
...
Re-runs one-stage mixed training for SimpleConvV2 (1.5 M) with a validation set that matches
the training mixture, because the original verdict measured the stopping rule
rather than the method.

`m3abl_mixed_scv2` scored 93.6 MSE / 5.42 MAE at 25 epochs and stopped there. The same
...
Hydra wiring — data `m3mix_matchedval` · model `simple_conv_v2` · loss `pit_mse` ·
metrics `rps`. Train with `python train.py experiment=m3mixv2_scv2`.
