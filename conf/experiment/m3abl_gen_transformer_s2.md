---
experiment: m3abl_gen_transformer_s2
training_config: conf/experiment/m3abl_gen_transformer_s2.yaml
batch: docs/experiments/generator-refined-labels.md
---

# `m3abl_gen_transformer_s2`

## Motivation

Ablation 2, stage 2 for the IF front-end transformer
(`simple_conv_v2_transformer_if`): the real fine-tune that closes the **does
the comb add anything on top of the generator?** question. Warm-started from
`m3abl_gen_transformer_s1`'s `best.ckpt`, it trains on the SAME real stream as
...
gap between this run and `m3cur_transformer_s2` is what the analytic comb adds.
Data `m3cur_s2`, model `simple_conv_v2_transformer_if`, loss `pit_mse`, metrics
...
experiment=m3abl_gen_transformer_s2` (after stage 1 uploads best.ckpt).
