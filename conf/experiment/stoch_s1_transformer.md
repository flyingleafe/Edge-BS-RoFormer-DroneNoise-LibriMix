---
experiment: stoch_s1_transformer
training_config: conf/experiment/stoch_s1_transformer.yaml
batch: docs/experiments/stochastic-transfer.md
---

# `stoch_s1_transformer`

## Motivation

Synthetic-only training whose target is transfer to the real frozen validation
split, on the transformer with the instantaneous-frequency front end trunk. The whole noise pool is the
...
The comparison row is `m3abl_comb_transformer_s1` — the analytic static comb as the
whole pool on the same trunk, the same optimizer, the same validation split and
the same three augmentation blocks. It reaches 2563 validation PIT-MSE, against
...
Data `stoch_s1`, model `simple_conv_v2_transformer_if`, loss `pit_mse`, metrics `rps`, batch 128
...
Train: `python train.py experiment=stoch_s1_transformer`.
