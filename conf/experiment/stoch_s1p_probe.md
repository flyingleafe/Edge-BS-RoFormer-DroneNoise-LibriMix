---
experiment: stoch_s1p_probe
training_config: conf/experiment/stoch_s1p_probe.yaml
...
A 55-minute triage copy of `stoch_s1p_floor` on the short GPU partition,
identical except for the run name, so its checkpoint can be scored per regime.
