---
experiment: salv2_hf0_comb_mix_crf
training_config: conf/experiment/salv2_hf0_comb_mix_crf.yaml
batch: docs/experiments/salv2-speech-and-objective-grid.md
---

# `salv2_hf0_comb_mix_crf`
...
Its BCE twin is [`salv2_hf0_comb_mix`](./salv2_hf0_comb_mix.md), identical in every other field, so the pair isolates the objective.
...
Hydra wiring — data `salv2_comb_mix` · model `harmof0_rps_l4` · loss `salv2_crf_r150` · metrics
`salience_layers_r150`. Train with `python train.py experiment=salv2_hf0_comb_mix_crf`.
