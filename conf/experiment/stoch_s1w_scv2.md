...
training_config: conf/experiment/stoch_s1w_scv2.yaml
...
`conf/online_mix/stoch_s1w_dload.yaml`): a smooth colored floor plus one
...
Arm H narrows the family to what a model can learn. Arms E and F sit at a
training loss of 445 after eighteen epochs while the analytic comb reaches 29 by
epoch 3 and the real regimes reach 5 to 20 — the family is about fifty times
harder to fit than anything else here, and a model that cannot fit its own
training data will not transfer. The width that costs the most and is least
real is the harmonic coherence: it was drawn uniformly, so a third of clips had
every harmonic of a rotor fading independently, which does not happen to a comb
driven by one shaft under one load. Coherence now comes from [0.6, 1.0], the
wander is halved, the floor sits further under the lines, and the augmentation
block is frequency scaling alone. The comparison row is `stoch_s1g_scv2`.
...
Data `stoch_s1w`, model `simple_conv_v2`, loss `pit_mse`, metrics `rps`, batch 128
...
Train: `python train.py experiment=stoch_s1w_scv2`.
