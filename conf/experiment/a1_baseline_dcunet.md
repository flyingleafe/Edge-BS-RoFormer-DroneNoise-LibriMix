---
experiment: a1_baseline_dcunet
training_config: conf/experiment/a1_baseline_dcunet.yaml
...
DCUNet baseline (Paper 1 comparison point), on DN-LM. See REPLICATION.md § A1.

Hydra wiring — data `dn_lm` · model `a1_baseline_dcunet` · loss `masked_mse` · metrics `separation_basic`. Train with `python train.py experiment=a1_baseline_dcunet`,
evaluate with `python eval.py experiment=a1_baseline_dcunet`.
