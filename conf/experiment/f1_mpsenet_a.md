---
experiment: f1_mpsenet_a
training_config: conf/experiment/f1_mpsenet_a.yaml
...
MP-SENet (parallel magnitude+phase, generator-only ~1.7M), Pass A (drone noises only) of the F1 blind speech-enhancement baseline
...
`python train.py experiment=f1_mpsenet_a`, evaluate with
`python eval.py experiment=f1_mpsenet_a metrics=separation_full`.
