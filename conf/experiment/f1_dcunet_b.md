---
experiment: f1_dcunet_b
training_config: conf/experiment/f1_dcunet_b.yaml
...
DCUNet (complex UNet; 2023 benchmark winner, continuity anchor), Pass B (all harmonic noises (category-uniform)) of the F1 blind speech-enhancement baseline
...
`python train.py experiment=f1_dcunet_b`, evaluate with
`python eval.py experiment=f1_dcunet_b metrics=separation_full`.
