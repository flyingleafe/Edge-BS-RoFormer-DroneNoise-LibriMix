...
def L1Freq_metric(
    ...
    reference = torch.from_numpy(reference).to(device)  # pyright: ignore[reportAssignmentType]
    estimate = torch.from_numpy(estimate).to(device)  # pyright: ignore[reportAssignmentType]

    # Compute STFT to get complex spectra
    reference_stft = torch.stft(reference, fft_size, hop_size, return_complex=True)  # pyright: ignore[reportArgumentType]
    estimated_stft = torch.stft(estimate, fft_size, hop_size, return_complex=True)  # pyright: ignore[reportArgumentType]
...
def NegLogWMSE_metric(
    ...
    reference = torch.from_numpy(reference).unsqueeze(0).unsqueeze(0).to(device)  # pyright: ignore[reportAssignmentType]
    estimate = torch.from_numpy(estimate).unsqueeze(0).unsqueeze(0).to(device)  # pyright: ignore[reportAssignmentType]
    mixture = torch.from_numpy(mixture).unsqueeze(0).to(device)  # pyright: ignore[reportAssignmentType]
...
def AuraSTFT_metric(
    ...
    reference = torch.from_numpy(reference).unsqueeze(0).to(device)  # pyright: ignore[reportAssignmentType]
    estimate = torch.from_numpy(estimate).unsqueeze(0).to(device)  # pyright: ignore[reportAssignmentType]
...
def AuraMRSTFT_metric(
    ...
    # Convert to torch tensors and add batch dimension
    reference = torch.from_numpy(reference).unsqueeze(0).float().to(device)  # pyright: ignore[reportAssignmentType]
    estimate = torch.from_numpy(estimate).unsqueeze(0).float().to(device)  # pyright: ignore[reportAssignmentType]
...
def bleed_full(
    ...
    return float(bleedless.cpu().numpy()), float(fullness.cpu().numpy())
