...
exit_code = pytest.main(
    [
        "tests/tracking/test_jhtr_dsp.py",
        "tests/models/test_jhtr.py",
        "-q",
        "--junitxml=results/cuda-tests.xml",
    ]
)
if exit_code:
...
result = dict(
    ...
    seconds=1,
    amp="float16",
    elapsed_forward_backward_s=time.perf_counter() - start,
    peak_allocated_bytes=torch.cuda.max_memory_allocated(),
    pass_small_cuda_smoke=True,
    matched_batch_profile=False,
)
