...
#show: report.with(
  ...
  abstract: [
    ...
    energy without restoring intelligibility. Training on *diverse* harmonic
    noise transfers positively to drone noise rather than diluting capacity.
    These floors gate later rotor-informed claims.
  ],
...
*Diversity verdict.* Comparing DCUNet trained on the drone-only pool (Pass A)
against the all-harmonic pool (Pass B), *on the drone validation set*, Pass B is
$>=$ Pass A at almost every SNR (@tbl:sisdr; e.g. Δ SI-SDR at −15 dB:
+5.0 → +7.6). Diverse harmonic-noise training *transfers* to drone noise rather
than diluting capacity — the transferable structure of rotating-source harmonics
outweighs the capacity cost.
...
their true ceiling is higher still. The positive diversity result is encouraging
for the broader programme: a single model trained across harmonic-noise families
generalises to drone noise, which supports scaling training data beyond drones.
