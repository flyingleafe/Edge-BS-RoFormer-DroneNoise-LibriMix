#!/usr/bin/env python3
"""Generate all figures for the Channel Generalization slide deck.

Adapted from the original ``slides/2026-06-09_channel_generalization/make_figures.py``.
Writes every figure into ``assets/`` (relative to this deck directory) so the
Touying deck can reference ``assets/<name>.png``.

The figures depend on synced ``results/`` (model checkpoints) and
...
# Repo root: writing/slides/<deck>/prepare.py -> parents[3]
...
FIG_DIR = Path(__file__).resolve().parent / "assets"
...
def make_ch0only_sc() -> None:
    ...
    predictor = load_predictor(
        f"simple_conv@{ROOT}/results/rps_exp_simple_conv/best_simple_conv.pt"
    )
    sample = next(
        s
        for s in load_input_set(str(ROOT / "datasets/DREGON-LM-V4/valid"))
...
def make_ch0only_v2() -> None:
    ...
    predictor = load_predictor(f"simple_conv_v2@{ROOT}/results/rps_exp_v2/best_simple_conv_v2.pt")
    sample = next(
        s
        for s in load_input_set(str(ROOT / "datasets/DREGON-LM-V4/valid"))
        if get_meta(s, "id") == "sample_00014"
...
def make_dynamic_8ch_v2() -> None:
    ...
    predictor = load_predictor(
        f"simple_conv_v2@{ROOT}/results/rps_8ch_v4_simple_conv_v2/best_simple_conv_v2.pt"
    )
    sample = next(
        s
        for s in load_input_set(str(ROOT / "datasets/DREGON-LM-V4/valid"))
...
def make_dynamic_8ch_sc() -> None:
    ...
    predictor = load_predictor(
        f"simple_conv@{ROOT}/results/rps_8ch_v4_simple_conv/best_simple_conv.pt"
    )
    sample = next(
        s
        for s in load_input_set(str(ROOT / "datasets/DREGON-LM-V4/valid"))
...
def make_tikz_images() -> None:
    ...
    report_dir = ROOT / "papers" / "simpleconv_variants_report"
    tex_files = [
