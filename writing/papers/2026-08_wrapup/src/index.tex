...
the comparison in the table holds the schedule fixed. \wip{PENDING: three
ablations --- comb-only pre-training, generator-only pre-training, and mixed
single-stage training --- isolate which ingredients are necessary; an early
signal is that comb-only pre-training transfers for the two convolutional
architectures and fails completely for the Transformer.}
