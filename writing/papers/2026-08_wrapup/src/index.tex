...
closes the failure on stopped rotors that plagued models trained on
cruise-heavy data: the best architecture improves from 52.5 to 22.1
validation PIT-MSE on real data alone, with the stopped-rotor error
falling from 11.8 to 3.4 rev/s. We also design an iterative classical
algorithm for
...
reduces to covering flight regimes the real corpus lacks. With the honest
real-data regime supplying that coverage directly, a synthetic pre-training
curriculum adds nothing for the best architecture (22.6 against 22.1),
retains a benefit only for the weakest one, and synthetic data mixed into
the real pool degrades every architecture regardless of the real recipe.
\end{abstract}
...
\item An evaluation of synthetic data as training material, from the
  ...
  covering flight regimes the real corpus lacks --- once the real regime
  supplies that coverage, the curriculum's benefit disappears for the best
  architecture and survives only where the architecture itself is weaker
  (Section 8).
\end{enumerate}
...
\toprule
Family & Method & \multicolumn{4}{c}{PIT MAE (rev/s)} & Inference cost \\
\cmidrule(lr){3-6}
 & & zero & ramps & cruise & all & per audio-second \\
\midrule
\multirow{6}{*}{Training-free}
 & PYIN ($\times$4, greedy) & 90.8 & 46.7 & 34.1 & 43.0 & $\sim$0.2 CPU-s \\
 & Cepstral (greedy) & 95.5 & 67.5 & 19.6 & 35.7 & $<$0.1 CPU-s \\
 & HPS (greedy) & 64.1 & 27.6 & 20.9 & 27.3 & $<$0.1 CPU-s \\
 & Matched-filter bank & 87.2 & 66.8 & 30.4 & 42.5 & $<$0.1 CPU-s \\
 & NMF, harmonic dictionary & 83.8 & 59.5 & \textbf{8.1} & 24.6 & $\sim$0.1 CPU-s \\
 & Inverse harmonic clustering & 68.8 & 28.3 & 16.3 & 24.6 & $\sim$2 CPU-s \\
\midrule
\multirow{2}{*}{Blind tracker}
 & ours, no refusal gates & 79.4 & 39.1 & \textbf{2.27} & 17.0 & \multirow{2}{*}{9.9 CPU-s} \\
 & ours, gates, refusal $\to$ 0 & \textbf{0.01} & 29.8 & 48.4 & 39.7 & \\
\midrule
\multirow{3}{*}{Salience (retrained)}
 & Basic Pitch & 34.0 & 13.3 & 31.7 & 29.5 & $\sim$0.5 CPU-s \\
 & multi-F0 CNN, standard grid & 52.8 & 21.0 & 4.0 & 12.5 & $\sim$2 CPU-s \\
 & multi-F0 CNN, wide fine grid & 48.2 & 16.1 & 4.7 & 11.7 & $\sim$2 CPU-s \\
\midrule
\multirow{3}{*}{Regression, R2 real-only}
 & convolutional (BiGRU head) & \textbf{3.4} & \textbf{4.2} & \textbf{2.35} & \textbf{2.72} & \multirow{3}{*}{$\sim$0.25 CPU-s} \\
 & Transformer head & 5.5 & 5.1 & 2.65 & 3.35 & \\
 & causal GRU head & 6.0 & 8.2 & 3.06 & 4.12 & \\
\midrule
\multirow{3}{*}{$+$ voicing gate, best front-end}
 & convolutional (IF) & 4.5 & 5.0 & 2.49 & 3.08 & \\
 & Transformer (magnitude) & 3.7 & 4.2 & 2.79 & 3.10 & \\
 & causal GRU (synchrosqueezed) & 4.6 & 6.1 & 2.67 & 3.38 & \\
\midrule
\multirow{2}{*}{Other trunks, R2 real-only}
 & Edge-BS-RoFormer & 34.5 & 12.3 & 3.21 & 8.40 & $\sim$0.6 CPU-s \\
 & Kalman linear attention & 6.0 & 4.3 & 3.51 & 3.92 & $\sim$0.3 CPU-s \\
\midrule
\multirow{4}{*}{Synthetic regimes (R3--R5)}
 & R3 curriculum, convolutional & 3.9 & 4.3 & 2.49 & 2.90 & \\
 & R3 curriculum, causal GRU & 4.9 & 5.5 & 2.79 & 3.42 & \\
 & R4 comb-only, causal GRU & 6.2 & 4.9 & 2.85 & 3.54 & \\
 & R5 mixed, convolutional & 16.1 & 9.1 & 5.20 & 7.11 & \\
\bottomrule
\end{tabular}
\caption{Motor-speed estimation on the frozen validation protocol: per-frame
permutation-invariant MAE pooled per flight regime (stopped rotors / ramps /
cruise) and over all frames, with approximate single-core inference cost.
Bold marks the best value per column. Neural rows are single runs; the
remaining cells of the regime grid (R3--R5 on the Transformer, R4 on the
convolutional model) are still training and join the table when they land.
\pending{fill the remaining regime cells}}
\label{tab:leaderboard}
\end{table*}

Three facts organize the table. First, the regression models win every
column except one: the blind tracker's gate-free cruise MAE (2.27 rev/s)
stays ahead of the best neural cruise cell (2.35), at roughly forty times
the inference cost. Second, the zero column separates methods that can
represent silence from methods that cannot: every training-free method
floors at its search-grid boundary, the salience models and the
Edge-BS-RoFormer light up on content-rich silence, and only the gated
blind tracker and the honest-regime compact models decide silence
correctly. Third, the honest real-only regime R2 is the best training
recipe in the table: the synthetic curriculum matches it without improving
it (2.90 against 2.72 all-frame MAE), and mixing synthetic data into the
real pool degrades every architecture.

The two alternative trunks confirm the pattern from opposite sides. The
Edge-BS-RoFormer, adapted from the speech-enhancement model of
\cite{liu2025edgebsrof}, learns only under regime R2 (under R1 its
validation error stays flat for the full budget); the trained model
reaches a competitive cruise cell (3.21 rev/s) and fails on stopped
rotors (34.5 rev/s), at roughly thirty times the parameter count of the
convolutional winner. The Kalman-linear-attention head, which replaces
the temporal head of the convolutional trunk, becomes a coherent model
under R2 (6.0 / 4.3 / 3.51 rev/s) and trails the bidirectional-GRU head
in every regime; Section~\ref{sec:discussion} returns to why a pooled
sequence mixer cannot reproduce the classical measurement update. Qualitative per-clip figures (rumble-heavy silence, a
stop--start transition, cruise) follow.
...
The Kalman-linear-attention result carries a design lesson. The classical
refinement pass reads the spectrogram at the harmonic positions its
current speed estimate predicts. The attention head receives features
pooled over frequency before its recurrence runs, so a state-conditioned
measurement is impossible, and the head can only filter what the pool
kept. The smallest architecture that matches the classical pass is a cell
that gathers complex spectrogram values at the positions its own
frequency state predicts and updates the state from the gathered phase
increments; we specify and train this cell in ongoing work.
...
curriculum improves the weaker architectures, while for the best
architecture an honest real-data regime --- full-envelope recordings with
explicit silence and a level floor --- reaches the same result from real
data alone. Synthetic data supplies coverage; once the real regime
supplies the same coverage, its value disappears, and merging it into the
real pool degrades every architecture.
