...
convolutional model) and the Edge-BS-RoFormer and Kalman-attention rows are
still training and join the table when they land. \pending{fill the
remaining regime cells and the two extra architecture rows}}
...
floors at its search-grid boundary, the salience models light up on
content-rich silence, and only the gated blind tracker and the
honest-regime neural models decide silence correctly. Third, the honest
real-only regime R2 is the best training recipe in the table: the
synthetic curriculum matches it without improving it (2.90 against 2.72
all-frame MAE), and mixing synthetic data into the real pool degrades
every architecture. Qualitative per-clip figures (rumble-heavy silence, a
