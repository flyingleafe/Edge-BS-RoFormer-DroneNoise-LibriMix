...
with close and intersecting speed trajectories, independent per-harmonic phase noise,
and a strong broadband component defeat both classical
least-squares harmonic decomposition and existing tacholess order
tracking methods. We propose a joint probabilistic model of the harmonic
and broadband components where phase noise enters through the increments
of instantaneous frequency and the broadband floor is an explicit smooth
spectral surface that charges a likelihood rent for the energy it absorbs.
Maximum a posteriori estimation in this model is performed by a block-coordinate
...
However, this is not the case. First, a quadcopter carries
...
component (turbulence, wind noise, etc.) is strong and non-stationary.
Existing tacholess order-tracking and
multi-pitch methods, built for single shafts or for well-separated
harmonic sources, do not survive this combination
(Sec.~\ref{sec:related}).

One may approach the problem from an optimization perspective: if we had
a measure of \emph{fitness} of the given set of rotor speed trajectories
to the given audio, we could use a generic non-convex optimization algorithm
to find the optimal solution in many cases. However, as we
show, straightforward ways to define such a measure -- such as measuring the
amplitudes of the harmonic components extracted via Vold-Kalman filtering from
the audio using the provided speeds -- do not reliably form a global maximum
at or near true rotational speeds provided by telemetry.
...
reviews related work. Section~\ref{sec:prelim} fixes notation and
states the two standard constructions the paper builds on --- the
harmonic-plus-noise model with its Whittle likelihood, and Vold--Kalman
order tracking. Section~\ref{sec:model} builds the signal and noise
...
the harmonic-plus-noise model~\cite{serra1990sms,stylianou2001hnm}, and,
recently, differentiable digital signal processing, where a neural
network predicts per-harmonic amplitudes and a filtered-noise floor end
to end~\cite{engel2020ddsp}. On the estimation side, Bayesian and
maximum-likelihood models of harmonic signals in colored noise are well
developed for pitch
estimation~\cite{christensen2009multipitch,nielsen2017fastf0}, and
smooth spectral floors are classically estimated by Whittle-likelihood
and cepstral methods~\cite{whittle1953,stoica2006cepstrum}. Our model
belongs to this family, with three features the application forces on
it: per-harmonic phase noise expressed in IF increments (rather than
...
arguments for model-order selection --- the Occam-factor
view of Bayesian model comparison~\cite{mackay1992bayesian}.

\emph{MAP source separation.} Viewed as source separation, our
decomposition is a maximum a posteriori split of one recording into
structured components, and it belongs to the classical family of
Gaussian variance models: each component is zero-mean Gaussian with a
structured spectral variance, inference reduces to Whittle likelihoods,
and the separation itself is a posterior (Wiener-type)
estimate~\cite{fevotte2009isnmf,duong2010fullrank,turner2014probtf}.
Two lessons from that literature recur in our design: model the
\emph{power} of whatever is phase-incoherent and marginalize its phase
--- the estimator that falls out is the conditional mean, whose known
artifacts (over-suppression between lines) have known remedies --- and
absorb non-stationarity that a Gaussian floor cannot carry with
heavier-tailed noise models~\cite{mogami2018ilrma}. The modern
general-sound branch of the same family replaces analytic priors with
learned generative models and posterior
sampling~\cite{jayaram2020basis,chung2023dps,scheibler2023diffsep,mariani2024msdm,lutati2024separatediffuse},
or trains discriminative separators under a mixture-consistency
constraint~\cite{wisdom2020mixit}. Those methods target sources with no
shared parametric structure, and none offers per-component
identifiability: the separations are perceptually plausible without
amplitude guarantees, which is exactly what a downstream noise model
cannot tolerate in a training target. Our setting makes the opposite
trade: the components are tied to a physical parameter --- the rotor
speed trajectories --- through a measured coherence law, and that tie is
what lets the decomposition double as a measure of the parameter. The
Bayesian harmonic-model
line~\cite{davy2006bayesharmonic,cemgil2006generative} is the closest
ancestor; we differ in the per-harmonic phase-noise law, the coupling of
four interleaved combs, and the use of the converged objective as a
trajectory fitness.
...
Two standard constructions carry the rest of the paper: the
harmonic-plus-broadband signal model with its Whittle likelihood, and
Vold--Kalman order tracking. We state both with the notation used
throughout. Discrete time is $n$ at sample rate $f_s$
($\Delta = 1/f_s$); short-time spectra are indexed by frequency bin $f$
and frame $t$; $D_p$ denotes the $p$-th order difference operator on a
uniformly sampled sequence, and $[\,\cdot\,]_+ = \max(\cdot, 0)$.

\subsection{Harmonic plus broadband noise, and the Whittle likelihood}
\label{sec:prelim-hpn}

The classical harmonic-plus-noise model~\cite{serra1990sms,
stylianou2001hnm} writes a signal as a sum of slowly modulated
harmonics over a stochastic residual,
\begin{equation}
  x[n] = \sum_{k=1}^{K} \operatorname{Re}\!\big[ a_k[n]\,
    e^{\,\mathrm{j} k \varphi[n]} \big] + n[n],
  \qquad
  \varphi[n] = 2\pi\Delta\!\!\sum_{m\le n}\!\! r[m],
  \label{eq:hpn}
\end{equation}
with complex envelopes $a_k$ that vary slowly against the carrier and a
fundamental phase $\varphi$ that is the running integral of a
fundamental frequency $r(t)$. The residual $n[n]$ is modeled as
zero-mean colored Gaussian noise with a power spectral density
$S(f,t)$ that is smooth in both arguments. For such noise the
negative log-likelihood of the observed short-time power
$P(f,t) = |X(f,t)|^2$ is, up to constants, the Whittle
form~\cite{whittle1953}
\begin{equation}
  -\log p\big(X \,\big|\, S\big) \;\simeq\;
  \sum_{f,t} \left[ \frac{P(f,t)}{S(f,t)} + \log S(f,t) \right].
  \label{eq:whittle}
\end{equation}
Both terms matter later. The first counts residual energy \emph{in
units of the local floor}, so energy at floor level costs the same
wherever it sits; the second charges for the floor's own level --- the
cell-wise unpenalized optimum is the maximum-likelihood variance
estimate $S^\ast = P$ --- so a floor cannot inflate for free.
Smoothness of $\log S$ is what makes the floor an estimator rather than
a tautology, and it is classically imposed by cepstral truncation or
thresholding~\cite{stoica2006cepstrum} or by running-median smoothing
across frequency; we use both, in that order.

\subsection{Vold--Kalman order tracking}
\label{sec:prelim-vk}

Order tracking extracts the harmonic (``order'') content of a rotating
machine given its shaft rate. The second-generation Vold--Kalman (VK)
filter~\cite{vold1993high,vold1997multi} takes the rate $r(t)$ as
\emph{known} --- from a tachometer --- forms the carrier
$\varphi$ as in \eqref{eq:hpn}, and estimates the envelopes of a set of
orders $\mathcal{M}$ (harmonic multiples $k_m$, possibly of several
shafts) as the penalized least-squares problem
\begin{equation}
  \min_{\{a_m\}} \;
  \sum_n \Big| x[n] - \!\!\sum_{m \in \mathcal{M}}\!\!
  \operatorname{Re}\big[ a_m[n]\, e^{\,\mathrm{j} k_m \varphi_{i(m)}[n]}
    \big] \Big|^2
  + \sum_m \rho_m^2\, \big\| D_p\, a_m \big\|^2 ,
  \label{eq:vk}
\end{equation}
whose two sums the VK literature calls the \emph{data equation} and the
\emph{structural equation}: the envelope must explain the signal at its
order's instantaneous frequency, and it must be smooth ($p = 2$
throughout this paper). The problem is convex and exactly solvable: after
demodulating by each carrier and decimating to an envelope rate
\fsenv{}, the normal equations are Hermitian positive-definite and
banded, with cross terms between orders that are negligible unless two
orders' instantaneous frequencies approach within the filter bandwidth
--- in which case the orders must be solved \emph{jointly}, the regime
multi-shaft VK was built for~\cite{vold1997multi} and the permanent
regime of a quadrotor's interleaved combs. The penalty weight $\rho_m$
is not a free knob: it maps one-for-one to a $-3$\,dB envelope bandwidth
$b$ through the filter's selectivity
characteristic~\cite{herlufsen1999characteristics,tuma2000characteristics},
\begin{equation}
  \rho(b) = \big( 2 \sin ( \pi b / \fsenv ) \big)^{-p},
  \label{eq:tuma}
\end{equation}
so a VK configuration is stated entirely in hertz. We reuse
\eqref{eq:tuma} as the single calibration relation for \emph{every}
smoothness penalty in this paper.

Two properties of \eqref{eq:vk} shape everything downstream. First, VK
is an \emph{envelope} estimator: the rate is an input, not an output.
Adaptive variants close the loop by feeding the demodulated envelope's
phase derivative back into the carrier~\cite{pan2007adaptive,
pan2010extended,jiang2024iavkf}; the capture range of any such loop is
bounded by the envelope bandwidth (Sec.~\ref{sec:capture}), which is
the arithmetic this paper's architecture is built around. Second, the
minimum of \eqref{eq:vk} over the envelopes --- the profiled projection
residual --- is the natural candidate for a measure of how well an
assumed rate fits the audio, and Sec.~\ref{sec:whynotls} is the
demonstration that this natural candidate fails on real drone
recordings; repairing it by embedding \eqref{eq:vk} in the Whittle
likelihood \eqref{eq:whittle} is the subject of
Sec.~\ref{sec:map}.
...
\subsection{The model as a joint distribution}
\label{sec:model-distributions}

We now state every stochastic ingredient formally; the MAP objective of
Sec.~\ref{sec:map} is nothing but the negative logarithm of the
posterior assembled here.

\emph{Observation.} Given the comb state, the residual
$x_c - \sum_{i,k}\operatorname{Re}[A_{c,i,k} e^{\mathrm{j}(\cdot)}]$ is
zero-mean colored Gaussian noise, independent across microphones, with
power spectral density $S_c(f,t)$; its likelihood is the Whittle form
\eqref{eq:whittle} applied per microphone.

\emph{Shaft rate error.} The rate error of rotor $i$ splits into a
rig-common part and a per-rotor part,
$\delta_i(t) = \bar\delta(t) + \tilde\delta_i(t)$, each a stationary
Ornstein--Uhlenbeck (OU) process,
\begin{equation}
  d\delta = -\frac{\delta}{\tau}\,dt
  + \sqrt{\frac{2\sigma^2}{\tau}}\, dW,
  \label{eq:ou}
\end{equation}
with variance $\sigma^2$ and correlation time $\tau$ (parameters
$(\bar\sigma,\bar\tau)$ and $(\tilde\sigma,\tilde\tau)$ respectively).
On the envelope time grid (step $\Delta_{\mathrm{e}} = 1/\fsenv$) this
is the AR(1) process
$\delta[j{+}1] = e^{-\Delta_{\mathrm{e}}/\tau}\,\delta[j] + w[j]$,
$w \sim \mathcal{N}\!\big(0,\, \sigma^2 (1 -
e^{-2\Delta_{\mathrm{e}}/\tau})\big)$.

\emph{Per-harmonic wobble.} Each $\nu_{i,k}$ is an independent OU
process with parameters $(\sigma_k, \tau_\nu)$. Integrated into a phase
as in \eqref{eq:phase-integrals}, an OU rate produces, over spans long
against $\tau_\nu$, phase diffusion with a Lorentzian line of
half-width (in Hz)
\begin{equation}
  \gamma_k = 2\pi\, \sigma_k^2\, \tau_\nu ,
  \label{eq:linewidth}
\end{equation}
and the measured law $\gamma_k \approx 0.6k$\,Hz
(Sec.~\ref{sec:model-measurements}) pins the product
$\sigma_k^2 \tau_\nu$ --- the single quantity the estimator needs.

\emph{Envelopes.} The prior on each complex envelope is the improper
Gaussian
$p(A_{c,i,k}) \propto \exp\!\big( -\rho_k^2 \lVert D_2 A_{c,i,k}
\rVert^2 \big)$: curvature is charged, amplitude is not (the prior is
invariant to adding any complex linear-in-time trend). This is a
deliberate choice --- the envelope carries the unknown per-harmonic
amplitude, which we refuse to bias --- and it has a consequence
developed in Sec.~\ref{sec:discussion}: an amplitude-free prior
provides no shrinkage, so the discipline that keeps an envelope from
absorbing floor energy must come entirely from the floor's own terms.

\emph{Floor.} The prior on the floor is a hard constraint:
$\log S_c(\cdot, t)$ must lie in the cepstrally band-limited family
(first $n_{\mathrm{cep}}$ quefrency coefficients), piecewise constant
over time blocks. Formally $R(\log S)$ in \eqref{eq:J} is the indicator
of this set; the implementation realizes it by projection (median
smoothing and cepstral truncation) rather than by a penalty, a
distinction that matters and is taken up in
...
$\psi = \{\psi_{i,k}\}$, and the floor $S = \{S_c(f,t)\}$. The
objective is the negative log posterior of
Sec.~\ref{sec:model-distributions}; one limit turns its OU priors into
quadratic penalties. Within an analysis window of length
$T_w$ (12\,s in our implementation) with
$T_w \ll \tau$, the AR(1) decay of \eqref{eq:ou} is negligible and the
rate-error \emph{increments} are white:
$\delta[j{+}1] - \delta[j] \sim \mathcal{N}(0, q)$ i.i.d.\ with
$q = 2\sigma^2 \Delta_{\mathrm{e}}/\tau$. Because every phase is the
integral of a rate \eqref{eq:phase-integrals}, the second difference of
the phase \emph{is} the rate increment,
\begin{equation}
  (D_2\, \theta_i)[j]
  = 2\pi\Delta_{\mathrm{e}} \big( \delta_i[j] - \delta_i[j{-}1] \big),
  \label{eq:d2-increment}
\end{equation}
so the window-local negative log prior of each integrated phase is a
\emph{second-difference (curvature) penalty},
\begin{equation}
  -\log p(\theta_i) \;\simeq\;
  \lambda_\theta \lVert D_2 \theta_i \rVert^2 ,
  \qquad
  \lambda_\theta = \frac{1}{2\, (2\pi \Delta_{\mathrm{e}})^2\,
    q_\theta},
  \label{eq:prior-limit}
\end{equation}
and identically for each $\psi_{i,k}$ with weight $\lambda_\psi(k)$
from $q_{\nu}(k)$, i.e.\ from the measured linewidth law through
\eqref{eq:linewidth}. We never quote the diffusion constants
themselves: each $\lambda$ is specified as the $-3$\,dB bandwidth of
the smoother it induces, through the calibration relation
\eqref{eq:tuma}, which is what makes every hyperparameter a bandwidth
in hertz. Writing $P_{c}(f,t)$ for the short-time residual power of
microphone $c$ after subtracting the harmonic reconstruction of
\eqref{eq:model}, the negative log posterior is, up to constants,
\begin{multline}
  \Jobj(A, \theta, \psi, S) =
  ...
  \label{eq:J}
\end{multline}
where $R(\log S)$ is the smoothness penalty on the log-floor and every
penalty weight maps one-for-one to a bandwidth by the VK selectivity
relation \eqref{eq:tuma}. The first sum is the Whittle likelihood
\eqref{eq:whittle} of the residual against the floor; the last
curvature sum is the structural equation of the VK problem
\eqref{eq:vk}. One calibration relation is thus shared by every
smoothness term in the system --- envelopes, shaft phases, per-track
phases --- so the hyperparameters of \eqref{eq:J} are bandwidths, all
in hertz, all interpretable, and all frozen across recordings
...
\emph{The floor as constraint versus penalty, and the amplitude-free
envelope prior.} Two deliberate simplifications of
Sec.~\ref{sec:model-distributions} interact, and their joint failure
mode is measurable. The floor's smoothness is implemented as a
projection outside the likelihood, so where the comb's mask blankets a
band the fitted $S$ is an extrapolation and can land below the true
local floor; and the envelope prior is amplitude-free, so nothing
shrinks an envelope that absorbs energy the under-fitted floor has
made expensive. On the outdoor recordings the result is visible: in
the band where only the fastest rotor's harmonics exist, the coherent
channel absorbs \pending{consolidate: $\approx 20\%$} of the original
power as a quasi-continuum, stripping the broadband channel
$\approx 2$\,dB below its level in the adjacent track-free band. The
principled repair is sequential: first fit $\log S$ \emph{inside} the
likelihood (penalized Whittle, with the comb explained by the model
rather than masked away), which prices dense-band energy correctly;
then replace the improper envelope prior at high orders with a proper
one --- a band-limited Gaussian line process with per-harmonic power
$H_k$ and the measured coherence bandwidth --- whose MAP estimate is a
Wiener smoother that takes a line's excess and leaves the floor. The
second step also removes the singularity of overlapping wide bands
(proper priors act as a ridge), at the honest price that the split of
genuinely shared twin energy then follows the priors rather than the
data. \pending{both repairs are design work in progress; the numbers
above are from the current implementation.}
