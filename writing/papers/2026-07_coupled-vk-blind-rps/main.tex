...
\title{Blind Tracking of Four Rotor Rates from Quadrotor Ego-Noise with a
  Capture-Range-Explicit Vold--Kalman Pipeline}
...
\begin{abstract}
Per-rotor rotational speed conditions modern drone-audition systems, yet
synchronized tachometer data is rarely recorded. We present a blind
estimator that recovers all four rotor-rate trajectories of a quadrotor
from onboard audio alone, including twin pairs separated by less than
$1\revps$. The method is staged---global comb-matched dynamic programming,
per-rotor decoupling, a coupled Vold--Kalman (VK) envelope solve, and a
single phase-increment maximum-likelihood smoothing pass---and every
stage's capture range is stated explicitly. Analysis shows that the
classical VK frequency-update loop has capture radius $B/(2k)$ and is
therefore inert under realistic blind initialization errors; the bandwidth
adaptation of the recent iterative adaptive VK filter (IAVKF) is an
implicit capture schedule with no harmonic-order dependence, and the
$k$-scaled bandwidth $B_k = kB_0$ makes the capture radius
order-invariant. A measured per-harmonic information law
($k^{2.0}$ indoor, $k^{1.5}$ outdoor, with no amplitude factor) replaces
the conventional Fisher weights. Evaluation uses a four-tier synthetic
ladder, two real corpora (DREGON, Matrice~100), a controlled-imprecision
initialization protocol shared by all methods, and baselines including
IAVKF and tacholess order tracking. Our chain holds an exact
initialization to $0.055\revps$, attains the proven $0.2\revps$ four-rotor
floor at 0\,dB, and improves cross-drone cruise error by 23\%; baseline
comparisons are \pending{baseline runs}.
\end{abstract}
...
\IEEEPARstart{T}{he} harmonic component of multirotor ego-noise---four
combs of tones at integer multiples of each rotor's shaft rate---is both
the strongest and the most structured part of the interference facing any
acoustic payload. When the per-rotor rate trajectories $r_i(t)$ are known,
that structure becomes usable: rotor-conditioned enhancement
networks~\cite{gulli2025rotorcond,liu2025edgebsrof}, rotor-driven noise
covariance estimation~\cite{yen2023ncm}, and ego-noise blind source
separation~\cite{wang2020bss} all consume rate side information. But
synchronized per-rotor telemetry is rarely recorded; of the corpora in
common use, DREGON~\cite{strauss2018dregon} is nearly unique in shipping
motor telemetry at all. Blind acoustic recovery of $r_i(t)$ is therefore
the enabling problem.

The problem is hard in ways single-shaft order tracking is not. Four combs
overlap in one spectrum; two of them typically form a \emph{twin pair}
under $1\revps$ apart (DREGON cruise: $\{74.0, 74.7, 85.3,
86.3\}\revps$), so their $k$-th harmonics stay within a filter bandwidth
of each other for whole analysis windows. Real flight audio decoheres:
per-harmonic phase wanders relative to the shaft phase, collapsing
long-horizon coherence even under telemetry-truth demodulation. Warmup
spectra contain only even (blade-passage) harmonics, making the octave
ambiguous, and takeoff ramps sweep $30\revps$ in seconds.

This paper contributes:
\begin{enumerate}
\item a blind staged pipeline with an explicit capture range for every
  stage, evaluated on DREGON and a cross-drone Matrice~100 corpus;
\item proof and threefold measurement that the classical VK frequency
  loop cannot operate blind, and a capture-range reading of published
  adaptive-VK methods, in particular IAVKF~\cite{jiang2024iavkf};
\item the $k$-scaled bandwidth $B_k = kB_0$, which makes the capture
  radius harmonic-order-invariant, and a measured per-harmonic
  information law (optimal weights $\propto k^{2.0}$/$k^{1.5}$ with no
  amplitude factor) that contradicts the deployed Fisher weighting;
\item a controlled-imprecision initialization protocol that puts all
  refiners on identical footing, and a label-free reconstruction referee
  that also audits the reference telemetry itself, detecting a drifting
  clock and a $0.7\%$ rate scale in one corpus's labels.
\end{enumerate}
