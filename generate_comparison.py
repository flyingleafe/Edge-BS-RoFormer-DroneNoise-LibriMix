...
DEFAULT_EVAL_DIR = get_results_path("evaluation")
DEFAULT_AUDIO_DIR = get_results_path("evaluation")
...
def load_model_data(evaluation_dir, model_names):
    ...
    for model_name in model_names:
        if model_name not in name_to_file:
            print(
                f"Warning: Model '{model_name}' not found. Available models: {list(name_to_file.keys())}"
            )
            continue
...
def prepare_plot_data(dataframes):
    ...
    for metric in metrics_to_plot:
        ...
        for model_name in combined_df["Model"].unique():
            ...
            # Group by SNR bin
            grouped = model_data.groupby("SNR_bin", observed=True)[metric].agg(
                ["mean", "std", "count"]
            )
            ...
            if snr_values:
                ...
                plot_data[metric][model_name] = {"snr": [], "mean": [], "std": []}
...
def create_comparison_plot(plot_data, model_names, output_path, figsize=(18, 8)):
    ...
    for metric, ylabel, title, ax in metrics_config:
        for model_name in model_names:
            ...
            std_vals = [
                data["std"][i] if pd.notna(data["std"][i]) else 0
                for i in range(len(data["std"]))
                if valid_mask[i]
            ]
...
def create_audio_comparison_plots(model_names, sample_ids, audio_dir, output_dir):
    ...
    for sample_id in sample_ids:
        ...
        for idx, (model_name, audio) in enumerate(audio_data.items()):
            ...
            ax_wav = axes[0, idx]
            ax_wav.plot(
                time_axis, audio, linewidth=0.5, color=MODEL_COLORS.get(model_name, "black")
            )
        ...
        for idx, (model_name, audio) in enumerate(audio_data.items()):
            ...
            img = librosa.display.specshow(
                S_db, x_axis="time", y_axis="hz", sr=sr, ax=ax_spec, cmap="plasma"
            )
...
Examples:
  ...
  python generate_comparison.py --models Edge-BS-RoFormer DCUNet --samples 00000 --output_dir presentations/full_comparison
    ...
    parser.add_argument(
        "--output_dir", type=str, required=True, help="Output directory for plots and tables"
    )
