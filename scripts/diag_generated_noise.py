...
from data_processing.dregon import load_dregon_timeframes, get_geometry as dregon_geom
from data_processing.michaels import load_michaels_timeframes, get_geometry as michaels_geom
...
model = PositionalHarmonicNoiseGen(sample_rate=SR, n_harmonics=N_HARM, use_diff_noise=True, cond_dim=cond_dim)
...
def generate_batch(drone: str, n: int) -> dict:
    ...
    mic_pos = np.array(mic_pos); rotor_pos = np.array(rotor_pos)
...
def amp_stats(audio, label):
    ...
    return {"label": label, "rms": rms, "peak": peak, "crest": crest,
...
for a in amp:
    print(f"{a['label']:<20} {a['rms']:>10.4f} {a['peak']:>10.4f} {a['crest']:>8.2f} {a['rms_per_ch_min']:>10.4f} {a['rms_per_ch_max']:>10.4f}")

print("\n=== RATIOS ===")
for gen_key, real_key, name in [("gen_dregon","real_dregon","DREGON"), ("gen_michaels","real_michaels","Michael's")]:
    gen_val = next(a for a in amp if a["label"] == gen_key)
    real_val = next(a for a in amp if a["label"] == real_key)
    print(f"  {name}: gen/real RMS = {gen_val['rms']/max(real_val['rms'],1e-10):.3f}, "
...
for rps, label in [(gen_d["rps"], "gen_dregon"), (gen_m["rps"], "gen_michaels")]:
    ...
    print(f"{s['label']:<20} {s['mean']:>10.2f} {s['std']:>10.2f} {s['p5']:>10.2f} {s['p95']:>10.2f}")
...
for tf in real_tfs_d:
    ...
    if rps is not None and rps.values is not None:
        v = rps.values
        print(f"    Mean={np.mean(v):.1f}, std={np.std(v):.1f}, min={np.min(v):.1f}, max={np.max(v):.1f}")
...
def avg_spectrum(audio, n_fft=2048, hop=512, max_samples=100):
    ...
    for i in range(n):
        for c in range(audio.shape[1]):
            ...
            spec = torch.stft(x, n_fft=n_fft, hop_length=hop, window=torch.hann_window(n_fft), return_complex=True)
...
print("\n=== SPECTRA ===")
specs = {}
for key, aud in [("gen_dregon", gen_d["audio"]), ("real_dregon", real_d_audio),
                  ("gen_michaels", gen_m["audio"]), ("real_michaels", real_m_audio)]:
...
for gn, rn, nm in [("gen_dregon","real_dregon","DREGON"), ("gen_michaels","real_michaels","Michael's")]:
...
with open(out_dir / "stats.txt", "w") as f:
    ...
    for rps, label in [(gen_d["rps"], "gen_dregon"), (gen_m["rps"], "gen_michaels")]:
        s = rps_stats(rps, label)
        f.write(f"{s['label']}: mean={s['mean']:.1f} std={s['std']:.1f} p5={s['p5']:.1f} p95={s['p95']:.1f}\n")
    f.write("\n=== SPECTRAL DISTANCE ===\n")
    for gn, rn, nm in [("gen_dregon","real_dregon","DREGON"), ("gen_michaels","real_michaels","Michael's")]:
        d = spectral_distance(specs[gn], specs[rn])
