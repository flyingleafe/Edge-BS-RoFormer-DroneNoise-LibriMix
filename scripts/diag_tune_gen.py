...
SR = 16000; DUR_S = 1.0; N_SAMPLES = 20; N_HARM = 100; DEVICE = "cuda:0"
...
def gen(drone, n, aggressiveness, amp_scale=1.0):
    ...
    rps_np = rps_synthesis.generate_intermittent_batch(n, DUR_S, SR, drone_profile=blend, aggressiveness=aggressiveness, rng=rng)
    if drone == "michaels":
        mp, rp = michaels_geom()
    else:
...
for agg in [1.0, 5.0, 10.0, 20.0]:
    for drone in ["dregon", "michaels"]:
        ...
        print(f"agg={agg:4.0f} {drone:10s}  RMS={rms:.4f} (×{amp_ratio:.2f})  RPS σ={rps_std:.1f} (×{rps_ratio:.2f})  RPS range={rps_range:.1f}")
...
for drone in ["dregon", "michaels"]:
    ...
    rps_std_all = rps.reshape(-1, rps.shape[-1]).std(axis=-1)
    rps_std = float(rps_std_all.mean())
    print(f"  {drone}: aggressiveness={BEST_AGG}, amp_scale={amp_scale:.3f}, RPS σ={rps_std:.1f} (target {REAL_RPS_STD[drone]:.1f})")
