...
for name, z_aud, r_aud in [("DREGON", gz_d, gr_d), ("Michael's", gz_m, gr_m)]:
    ...
    print(f"\n{name}:")
    print(
        f"  Zero-phase RMS:   mean={z_rms.mean():.4f}  std={z_rms.std():.4f}  min={z_rms.min():.4f}  max={z_rms.max():.4f}"
    )
    print(
        f"  Random-phase RMS: mean={r_rms.mean():.4f}  std={r_rms.std():.4f}  min={r_rms.min():.4f}  max={r_rms.max():.4f}"
    )
    ...
    print(
        f"  Per-sample correlation (zero vs random): mean={corr.mean():.3f}, std={corr.std():.3f}"
    )
    ...
    def avg_spec(audio):
        ...
        for i in range(min(20, audio.shape[0])):
            for c in range(audio.shape[1]):
                ...
                s = (
                    torch.stft(x, 2048, 512, window=torch.hann_window(2048), return_complex=True)
                    .abs()
                    .mean(-1)
                    .numpy()
                )
    ...
    spec_dist = float(
        np.mean(np.abs(20 * np.log10(z_spec + 1e-10) - 20 * np.log10(r_spec + 1e-10)))
    )
...
for name, aud in [("DREGON", gr_d), ("Michael's", gr_m)]:
    per_sample_rms = np.sqrt(np.mean(aud**2, axis=(-2, -1)))
    print(
        f"  {name}: per-sample RMS mean={per_sample_rms.mean():.4f}, min={per_sample_rms.min():.4f}, max={per_sample_rms.max():.4f}"
    )
