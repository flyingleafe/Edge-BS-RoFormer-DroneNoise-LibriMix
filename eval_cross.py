...
from losses.pit import pairwise_mse, pit_mse_loss
...
def main() -> None:
    ...
    for tag, ds, indices in [("old", old_ds, old_indices), ("v2", new_ds, new_indices)]:
        for idx in indices:
            ...
            item = ds[idx]
            audio = item[0].unsqueeze(0).to(device)
            rps_target = item[1].numpy()

            # Save ground truth
            np.save(out_dir / "rps_target.npy", rps_target)
            sf.write(out_dir / "mixture.wav", audio.cpu().squeeze().numpy(), 16000)
            sf.write(
                out_dir / "vocals.wav", sf.read(os.path.join(sample_dir, "vocals.wav"))[0], 16000
            )
            sf.write(
                out_dir / "noise.wav", sf.read(os.path.join(sample_dir, "noise.wav"))[0], 16000
            )
    ...
    for sample in all_samples:
        ...
        audio_track = tf["audio"]
        # audio data is (channels, N) — axis 0 = channels, time last
        audio_data = np.asarray(audio_track.data)
        n_channels = audio_data.shape[0] if audio_data.ndim > 1 else 1
        total_duration = audio_track.duration
        print(f"  Duration: {total_duration:.1f}s, channels: {n_channels}")

        # Use channel 0 for simplicity
        ch = 0
        audio_np = audio_data[ch, :] if audio_data.ndim > 1 else audio_data
        audio_full = torch.from_numpy(audio_np.astype(np.float32))

        # Extract command RPS (cleaned) for ground truth.  Motor data is
        # time-last (4, M) — the old ``.T`` here predated that convention fix.
        motor_key = "motors_command" if "motors_command" in tf else "motors_measured"
        rps_full = None
        if motor_key in tf:
            motor = tf[motor_key]
            if motor.data is not None:
                command = np.asarray(motor.data).copy()  # (4, M)
                command_cleaned = clean_command_spikes(command)
                rps_full = torch.from_numpy(command_cleaned.astype(np.float32))  # (4, T_motor)
        ...
        all_preds: dict[str, list[np.ndarray]] = {name: [] for name in loaded_models}
        all_targets_rps = []
        window_starts = []
        ...
        if rps_full is not None and len(all_targets_rps) > 0:
            ...
            for model_name in loaded_models:
                ...
                for w in range(len(preds)):
                    ...
                    loss = cast(torch.Tensor, pit_mse_loss(p, t))
                    pit_mses.append(loss.item())
