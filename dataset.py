...
class MSSDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        self.instruments = config.training.instruments  # List of instruments to separate
        ...
        self.min_mean_abs = (
            config.audio.min_mean_abs
        )  # Minimum mean absolute value (for filtering silent chunks)
    ...
    def load_random_mix(self):
        ...
        for instr in self.instruments:
            ...
            if (
                self.aug
                and not load_rps
                and "mixup" in self.config["augmentations"]
                and self.config["augmentations"].mixup
            ):
                mixup = [s1]
    ...
    def augm_data(self, source, instr):
        ...
        # 1. Channel shuffle
        if (
            "channel_shuffle" in augs
            and augs["channel_shuffle"] > 0
            and random.uniform(0, 1) < augs["channel_shuffle"]
        ):
        ...
        # 2. Random inverse
        if (
            "random_inverse" in augs
            and augs["random_inverse"] > 0
            and random.uniform(0, 1) < augs["random_inverse"]
        ):
            source = source[:, ::-1].copy()
        ...
        if (
            "random_polarity" in augs
            and augs["random_polarity"] > 0
            and random.uniform(0, 1) < augs["random_polarity"]
        ):
            source = -source.copy()
        ...
        if (
            "pitch_shift" in augs
            and augs["pitch_shift"] > 0
            and random.uniform(0, 1) < augs["pitch_shift"]
        ):
        ...
        if (
            "seven_band_parametric_eq" in augs
            and augs["seven_band_parametric_eq"] > 0
            and random.uniform(0, 1) < augs["seven_band_parametric_eq"]
        ):
        ...
        # 6. Tanh distortion
        if (
            "tanh_distortion" in augs
            and augs["tanh_distortion"] > 0
            and random.uniform(0, 1) < augs["tanh_distortion"]
        ):
        ...
        # 7. MP3 compression
        if (
            "mp3_compression" in augs
            and augs["mp3_compression"] > 0
            and random.uniform(0, 1) < augs["mp3_compression"]
        ):
        ...
        # 8. Gaussian noise
        if (
            "gaussian_noise" in augs
            and augs["gaussian_noise"] > 0
            and random.uniform(0, 1) < augs["gaussian_noise"]
        ):
        ...
        # 9. Time stretch
        if (
            "time_stretch" in augs
            and augs["time_stretch"] > 0
            and random.uniform(0, 1) < augs["time_stretch"]
        ):
        ...
        # 10. Reverb effect
        if (
            "pedalboard_reverb" in augs
            and augs["pedalboard_reverb"] > 0
            and random.uniform(0, 1) < augs["pedalboard_reverb"]
        ):
        ...
        # 11. Chorus effect
        if (
            "pedalboard_chorus" in augs
            and augs["pedalboard_chorus"] > 0
            and random.uniform(0, 1) < augs["pedalboard_chorus"]
        ):
        ...
        # 12. Phaser effect
        if (
            "pedalboard_phazer" in augs
            and augs["pedalboard_phazer"] > 0
            and random.uniform(0, 1) < augs["pedalboard_phazer"]
        ):
            rate_hz = random.uniform(
        ...
        if (
            "pedalboard_distortion" in augs
            and augs["pedalboard_distortion"] > 0
            and random.uniform(0, 1) < augs["pedalboard_distortion"]
        ):
        ...
        if (
            "pedalboard_pitch_shift" in augs
            and augs["pedalboard_pitch_shift"] > 0
            and random.uniform(0, 1) < augs["pedalboard_pitch_shift"]
        ):
        ...
        if (
            "pedalboard_resample" in augs
            and augs["pedalboard_resample"] > 0
            and random.uniform(0, 1) < augs["pedalboard_resample"]
        ):
        ...
        if (
            "pedalboard_bitcrash" in augs
            and augs["pedalboard_bitcrash"] > 0
            and random.uniform(0, 1) < augs["pedalboard_bitcrash"]
        ):
        ...
        if (
            "pedalboard_mp3_compressor" in augs
            and augs["pedalboard_mp3_compressor"] > 0
            and random.uniform(0, 1) < augs["pedalboard_mp3_compressor"]
        ):
    ...
    def __getitem__(self, index):
        ...
        res = self.load_random_mix() if self.dataset_type in [1, 2, 3] else self.load_aligned_data()

        # Randomly adjust loudness for each track
        if (
            self.aug
            and "loudness" in self.config["augmentations"]
            and self.config["augmentations"]["loudness"]
        ):
        ...
        if (use_rps or load_rps) and num_rotors > 0:
            ...
            getattr(self.config.audio, "n_fft", 2048)
            ...
            if (
                load_rps
                and hasattr(self, "_last_sample_path")
                and self._last_sample_path is not None
            ):
