...
def _lowpass_noise(rng: np.random.Generator, n: int, fs: float, bw_hz: float, rms: float):
...
def make_fixture(seed: int = 0) -> dict[str, object]:
    """A recording that obeys the v3 model, with every term known.

    The LABELS are a smooth trajectory (what telemetry gives). The TRUTH is
    those labels plus a shaft-rate error, so ``theta`` is exactly the phase the
    decomposition has to recover, and the per-track ``psi`` grows with ``k`` the
    way the linewidth law says. The floor is smooth and colored, not white,
    which is the second thing v2 assumes and does not have.
    """
    ...
    dr_rig = _lowpass_noise(rng, n_t, SR, SHAFT_BW_HZ, SHAFT_REV_S)
    theta = np.zeros((N_ROT, n_t))
    for i in range(N_ROT):
        dr = dr_rig + _lowpass_noise(rng, n_t, SR, SHAFT_BW_HZ, 0.2 * SHAFT_REV_S)
    ...
    for i in range(N_ROT):
        for kk in ks:
            psi = _lowpass_noise(rng, n_t, SR, min(0.6 * kk, 8.0), PSI_RMS_PER_K * kk)
