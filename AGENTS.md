...
| `src/framespec/` | The frame-shape vocabulary (`FrameSpec`, `SeriesSpec`, `TimeKind`) | Leaf package with no internal imports, so `losses`/`metrics` can sit below `tasks`. `tasks/spec.py` is a back-compat re-export |
