...
| `scripts/` | Standalone scripts not on the train/eval path | Dataset materialization/publishing (`derive.py` — the single driver), stream sanity checks (`check_stream.py`, `check_experiment_configs.py`), protocol evaluators (`rps_eval.py`, `beatvk_eval.py`), benchmarks. Remote jobs and result sync go through omnirun. |
