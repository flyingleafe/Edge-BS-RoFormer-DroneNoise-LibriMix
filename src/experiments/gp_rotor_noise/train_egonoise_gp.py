...
from data_processing.egonoise_gp import (
    FS,
    EgonoiseGPConfig,
    _make_gp,
)
from data_processing.egonoise_gp import (
    EgonoiseGPModel as _EgonoiseGPModelBase,
)
from experiments.gp_rotor_noise import jasa_gp as J

# Inference core (FS, EgonoiseGPConfig, _make_gp, the base EgonoiseGPModel)
# lives in data_processing.egonoise_gp so the online-mix ``kind: gp`` source
# never imports the experiments sandbox. This module keeps the training half.
__all__ = ["FS", "DRONES", "EgonoiseGPConfig", "EgonoiseGPModel"]
...
# ════════════════════════════════════════════════════════════════════════════
# The GP itself: training half on top of the data_processing inference core
# ════════════════════════════════════════════════════════════════════════════
class EgonoiseGPModel(_EgonoiseGPModelBase):
    """Per-drone GP: fit on the rps sweep, predict coeffs at (mic xyz, rps).

    The inference core (state, ``predict_coeffs``, ``save``/``load``) is
    :class:`data_processing.egonoise_gp.EgonoiseGPModel`; this subclass adds
    the sweep-fitting (``fit``) and rendering (``synthesize``) halves, which
    depend on :mod:`.jasa_gp`.
    """
    ...
    # ── synthesis (predict_coeffs/_broadband inherited from the base) ───────
    def synthesize(
        self,
        mics: np.ndarray,
        rps,
