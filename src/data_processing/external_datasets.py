...
# -- SPCUP19 egonoise: 10 heterogeneous student-team drone rigs (DREGON/INRIA) --
#
...
_SPCUP19_TEAMS: dict[str, dict[str, Any]] = {
...
def _spcup19_condition(name: str) -> str | None:
    low = name.lower()
    table = (
...
def _spcup19_frame(
    ...
    *,
    ...
    rid = f"{team}__{_safe_key(rid_suffix)}"
    extra: dict[str, Any] = {
        "raw_relpath": relpath,
    ...
    return _safe_key(rid), _audio_frame(audio_ct, int(sr), meta)
...
# ─── AVQ (audio-visual quadrotor: onboard 8-mic array + speech + DOA/VAD) ────────

_AVQ_URL = "https://webspace.eecs.qmul.ac.uk/lin.wang/download/avq.zip"


def _avq_mic_pos(session_dir: Path) -> np.ndarray | None:
...
def _avq_session_info(session_dir: Path) -> dict[str, Any]:
...
def _avq_angle_vad(seq_dir: Path) -> tuple[np.ndarray | None, str]:
...
def _avq_channels(seq_dir: Path) -> tuple[np.ndarray, int] | None:
    ...
    sr0: int | None = None
    for w in wavs:
        audio_ct, sr = _read_audio_file(w)  # mono file -> (1, T)
...
def build_avq(raw_dir: Path) -> Iterator[tuple[str, td.Frame]]:
    """AVQ = onboard 8-mic quadrotor array recording speech under ego-noise, with
    synchronized-video DOA + VAD ground truth (QMUL, Lin Wang). One recording
    Frame per sequence: 8-ch ``audio`` (native 44.1 kHz) + ``mic_pos`` +
    ``angle_vad`` (labeled sequences) + rich per-session meta (AV calibration,
    readme). The bulky/opaque blobs (video, cameraParams.mat, .docx) are kept
    byte-exact in the companion raw dataset ``AVQ-raw``, not here."""
    root = Path(raw_dir)
    ...
    for sess_dir in session_dirs:
        ...
        mic_pos = _avq_mic_pos(sess_dir)
        info = _avq_session_info(sess_dir)
        seq_dirs = sorted(
            p for p in sess_dir.iterdir() if p.is_dir() and p.name.lower().startswith("seq")
        )
        for seq_dir in seq_dirs:
            got = _avq_channels(seq_dir)
            if got is None:
                continue
            audio, sr = got
            angles, ang_desc = _avq_angle_vad(seq_dir)
            ...
            yield _safe_key(rid), td.Frame(entries)
...
EXTERNAL_SPECS: dict[str, ExternalDataset] = {
    ...
    "SPCUP19-egonoise": ExternalDataset(
        name="SPCUP19-egonoise",
        download=DownloadSpec("http", {"urls": _SPCUP19_URLS}, extract=True),
        builder=build_spcup19_egonoise,
        provenance={
    ...
    "AVQ": ExternalDataset(
        name="AVQ",
        download=DownloadSpec("http", {"urls": {"avq.zip": _AVQ_URL}}, extract=True),
        builder=build_avq,
        fields=("audio", "mic_pos", "angle_vad"),
        provenance={
