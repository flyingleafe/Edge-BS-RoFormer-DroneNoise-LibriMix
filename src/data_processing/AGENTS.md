...
| `rotor_spectral_model.py` | `StaticCombNoisePool` — analytic static-comb noise source (`kind: static_comb`) — plus `FixedCombSpec`/`render_fixed_comb`, the frozen-profile, RPS-independent-amplitude variant the generator probe needs (a per-clip random profile is unpredictable from RPS and would mask the effect measured). |
