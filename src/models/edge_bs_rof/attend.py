...
from collections import (
    namedtuple,  # Import namedtuple from collections module to create named tuples for storing configuration data
)
from functools import (
    wraps,  # Import wraps decorator to preserve decorated function's original metadata (e.g., function name, docstring)
)
...
from packaging import (
    version,  # Import version module from packaging library for version comparison operations
)
from torch import (  # Import neural network module nn and Einstein summation function einsum for concise tensor operations
    einsum,
    nn,
)
...
class Attend(nn.Module):
    ...
    def __init__(
        self,
        dropout=0.0,  # Dropout ratio in attention mechanism
        ...
        self.attn_dropout = nn.Dropout(
            dropout
        )  # Create Dropout layer based on given dropout ratio for random dropping of attention weights
    ...
    def flash_attn(self, q, k, v):
        ...
        with sdpa_kernel(backend):
            out = F.scaled_dot_product_attention(
                ...
                if self.training
                else 0.0,  # Apply dropout during training, disable during inference
            )
