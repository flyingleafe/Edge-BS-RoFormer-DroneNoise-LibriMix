...
class DCUNet(nn.Module):
    ...
    def __init__(self, config):
        ...
        for t in dec_spec:
            if len(t) == 7:
                self.decoders.append(
                    Decoder(
                        t[0], t[1], t[2], t[3], output_padding=t[4], padding=t[5], last_layer=t[6]
                    )
                )
            else:
                self.decoders.append(
                    Decoder(t[0], t[1], t[2], t[3], output_padding=t[4], padding=t[5])
                )
        ...
        self.predict_rps = _get_rps_config(config).get("predict_rps", False) or config.get(
            "predict_rps", False
        )
