...
The frame-shape vocabulary shared by datasets, models, losses and metrics
(a leaf package with no internal dependencies — canonical home of what was
``tasks.spec``). A *task* is the function type of a model: which entries
(and what kind of series) it consumes and produces. ``FrameSpec`` describes
the shape of a Frame; :func:`check_subsumes` verifies that what one stage
provides covers what the next stage requires — before any GPU time is spent.
