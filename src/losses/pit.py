...
# ─── Permutation alignment (numpy, evaluation/plotting) ─────────────────────
...
def align_rps_to_gt(pred: np.ndarray, gt: np.ndarray) -> np.ndarray:
    ...
    searching all rotor assignments (see :func:`tasks.rps_prediction.evaluate`'s
    ``pit`` branch). **Any plot that overlays a prediction on the ground truth
    must apply the same matching**, otherwise rotors appear swapped even when
