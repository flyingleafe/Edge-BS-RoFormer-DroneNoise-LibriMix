...
| `salience_layers.py` | `layer_pit_bce` / `LayerPITSalienceBCELoss` — BCE over **one Gaussian salience layer per rotor**, permutation invariant over the `R!` layer assignments. Replaces `salience.py`'s shared-map target for the harmonic ports, and the reason is a measurement, not taste: `models.salience_crf` encoded real training telemetry into the shared map and decoded it back — a PERFECT target, no model — and lost **8.24 rev/s** on average with 39-45% of frames more than half a bin off; the layer/Gaussian/CRF pair loses 2.22e-16. That 8.24 is an **oracle floor** that every shared-map row was measuring alongside its architecture. **`pos_weight` must stay 1**: weighted BCE against a soft target `p` is minimized at `w p / (w p + 1 - p)`, not at `p`, which saturates the bins around the peak and flattens the parabola the readout fits — the imbalance correction and the exact readout are directly opposed |
